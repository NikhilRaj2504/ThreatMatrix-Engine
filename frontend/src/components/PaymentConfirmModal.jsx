            ...
            <>
              ...
              <div className="p-4 rounded-xl bg-[#0B101E] border border-[#262F43] text-center">
                ...
                <div className="text-3xl font-extrabold font-mono text-[#F8FAFC] mt-1">
                  ₹{assessment?.detailed_signals?.transaction?.amount_ratio_avg ? `${(assessment?.scores_breakdown?.transaction_risk || 50000).toLocaleString('en-IN')}` : '50,000'}
                </div>
                <div className="text-xs text-[#94A3B8] mt-1 flex items-center justify-center gap-1">
                  <span>To:</span>
                  <span className="font-mono text-[#00D2D3] font-bold">{assessment?.detailed_signals?.entity?.account_holder || 'Receiver UPI'}</span>
                </div>
              </div>
